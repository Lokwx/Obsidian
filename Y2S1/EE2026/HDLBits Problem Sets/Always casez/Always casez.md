...
always @(*) begin
    casez (in[3:0])
        4'b???1: out = 0;   // in[3:1] can be anything
        4'b??1?: out = 1;
        4'b?1??: out = 2;
        4'b1???: out = 3;
        default: out = 0;
...
### Pointers
1. Ensure that `endcase` is present, regardless of `case/casez` is used
2. Ensure that the bit length is correct e.g. `8'bzzzzzzz0`